...
ggstatsplot::ggpiestats(
    data = ds_selected,
    x = sex_male,
    y = adhd_parent, results.subtitle = FALSE, proportion.test = FALSE)
```
...
```{r}
chisq.test(ds_selected$sex_male,ds_selected$adhd_parent, correct = F) %>% pander(., split.table = Inf, caption = "")
```
...
```{r }
table(ds_selected$sex_male,ds_selected$adhd_parent) %>% DescTools::Rev(.) %>% 
  epiR::epi.2by2(.)
```
...
```{r}
mod_logistico_simples <- glm(adhd_parent ~ sex_male, family = binomial, data = ds_selected)
sjPlot::tab_model(mod_logistico_simples, transform = NULL, show.ci = F, digits = 3)
```
...
```{r}
mod_logistico_maior <- glm(adhd_parent ~ sex_male + alcohol, family = binomial, data = ds_selected)
sjPlot::tab_model(mod_logistico_simples, mod_logistico_maior, transform = NULL, show.ci = F,digits = 3)
```
...
<div class="mirror">✂ Gabarito: 1-a; 2-d; 3-b; 4-e
<div>
