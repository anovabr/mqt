...
Essa tabela é particularmente útil e permite explorar possíveis associações entre as variáveis utilizando o teste Qui-quadrado de independência. Os resultados foram significativos, $X^2(1) = 3.398, p = 0.065$, indicando pela associação entre <u>sexo</u> e <u>risco</u>. No entanto, como o Qui-quadrado não trabalha com a ideia de variável independente e dependente, seus resultados não indicam em quais células os valores esperados e observados são significativamente diferentes.
...
Esses dois resultados são importantes, mas seriam interpretados separadamente. Eles concluiriam que existe uma associação significativa (com $\alpha < 0.1$) entre o sexo do participante e estar ou não no grupo de risco problemas psicológicos, bem como uma chance aumentada das mulheres apresentarem baixos recursos psicológicos quando comparados aos homens.  

Ao modelar esses dados por uma regressão logística, automaticamente se esta (1) realizando um teste Qui-quadrado de independência, com a vantagem de definir claramente uma variável independente e outra dependente, bem como se está (2) computando os OR dos preditores. Conceitualmente:  

\[\underbrace{ln\left ( \frac{Risco}{1-Risco} \right)}_\text{logit} = b_0 + b_1X_Sexo\]
...
Por sua vez, o Odds Ratio calculado previamente pela tabela de contingência também é encontrado na tabela de resultados da regressão logística ao se fazer $\exp(b)$. 
