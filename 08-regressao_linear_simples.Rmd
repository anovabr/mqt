...
```{r}
include_graphics("raw_img/cap_reg_x_y.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_x_y2.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_x_y_SSR.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_x_y_SSE.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_x_y_SST.png")
```
...
```{r } 
include_graphics("raw_img/cap_reg_x_y_SSR_SSE_SST.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_r2.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_r2_1.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_multipla_ortogonal.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_multipla.png")
```
...
```{r}
include_graphics("raw_img/cap_reg_multipla_colinearidade.png")
```
...
A Regressão linear é uma técnica estatística utilizada que além de verificar o relacionamento funcional entre duas variáveis, permite criar uma equação que verifique o quanto uma variável varie em função de outra.
...
(ii) Explicar a variabilidade da variável dependente (Y) em função da variável independente (X). Tecnicamente, ambas as utilidades são virtualmente iguais.

A Regressão linear simples pode ser vista a partir de um incremento ou avanço dos modelos de correlação. Dessa maneira, os aspectos correlacionais devem (e podem) ser inicialmente investigados. Conforme já exposto, o gráfico de dispersão auxilia na visualização da relação entre as variáveis.
...
A correlação entre ambas as medidas expressa a força e a direção que elas possuem. Enquanto a força é interpretada em `fraca` (0.1), `moderada` (0.3) ou `forte` (0.5), a direção pode ser positiva ou negativa, a depender do sinal. 
...
ggplot(dados_brasil, aes(x = bsq_soma, y = eat_soma)) +
  ...
  labs(x = "Resultados da Escala BSQ-34", y = "Resultados da Escala EAT-26", 
       title = "Correlação entre o BSQ-34 e o EAT-26") +
  geom_abline(slope = c(rnorm(10,.1,0.08), rnorm(10,0.1,0.7)),
              color = 1:20)
...
Repare que agora o valor previsto ($\hat{y}$) depende de duas constantes (`intercepto ou coeficiente linear` e `inclinação ou coeficiente angular`) e uma variável (x). Apenas por uma questão de simbologia, três coisas são feitas com a equação:  
(i) Os símbolos são alterados. Agora $a = b_0$ e $b = b_1$. A alteração de simbologia não altera em nada os cálculos.  
(ii) Como eu sei que essa reta vai estimar os valores reais $y$, letras minúsculas ou um chapéu sobre as letras será utilizado em vez das letras maiúsculas ou gregas.  
...
No entanto, entre o valor real de y (os pontos no gráfico) e os valores obtidos pela minha equação, haverá uma certa quantidade de erro de estimativa ($e_i$). Ou seja, os valores estimados de cada participante ($i$) vão ter uma quantidade de erro ($e_i$). Dessa forma, é possível pensar que os valores reais agora possuem uma porção de erro:

$$y_i = a + bx+\underbrace{e_i}_\text{aleatório}$$

É possível traçar milhões de retas, mas para encontrar a reta que minimiza os erros, é necessário discriona-lo.

$$e_i = y_i - \hat{y_i}$$
$$e_i = y_i - (b_0 + b_1X{_1}_i) \\ =  y_i - b_0 - b_1X{_1}_i$$


Quando se minimizar o erro ($\epsilon_{i}$) em relação ao $b_0$ e do $b_1$, eu vou conseguir ter os coeficientes da função que possibilitam que eu contrua a reta mais próxima a todos os pontos. O método utilizado para isso é o Mínimos Quadrados Ordinários (em inglês, Ordinary Least Squares -- OLS):

As derivada serão igualadas a 0:

$$\frac{\partial f}{\partial \beta_0}, \frac{\partial f}{\partial \beta_1} = 0$$
...
ggplot(dados_brasil, aes(x = bsq_soma, y = eat_soma)) +
  ...
  labs(x = "Resultados da Escala BSQ-34", y = "Resultados da Escala EAT-26", 
       title = "Correlação entre o BSQ-34 e o EAT-26") +
  geom_abline(intercept = b0, slope = b1) +
  geom_vline(xintercept = mean(dados_brasil$bsq_soma)) +
  geom_hline(yintercept = mean(dados_brasil$eat_soma))


```


Ainda aproveitando o gráfico, agora torna-se mais simples de notar os dados reais, a linha de regressão e as distâncias (resíduos) entre os pontos reais e os previstos.
...
Posto isso, agora vamos, computacionalmente e analiticamente, passo a passo na regressão linear simples. A função `lm` é a nativa para isso. Ela precisa da variável dependente e da variável independente, tal como abaixo.
...
Os resultados do `intercepto` ($b_0$) e da `inclinação` $b_1$ já foram analisados anteriormente, mas agora há 4 outros resultados que serão descritos. Em **primeiro momento**, é necessário verificar o ajuste do modelo e isso é feito pela linha `F-statistic`. Existem ao menos duas formas de entender essa etapa e os resultados obtidos. É possível chegar a eSSR resultado (I) comparando os resultados do modelo em questão com um modelo em que apenas o intercepto é utilizado para prever todos os valores ou (ii) verificando o quanto o modelo em questão consegue explicar da variabilidade dos dados. As duas formas serão vistas, a começar pela primeira.
...
Tanto o modelo compacto quanto o modelo aumentado geram uma quantidade de erro. A Soma dos Quadrados dos Erros (SSE) de ambos os modelos é a métrica utilizada para eSSR indicador. No R, é chamado de por Residual Sums-of-Squares (RSS).
...
Quão maior o valor da SSE, pior. No Modelo Compacto (intercepto-apenas), esse valor é de 299560.7. A conta é relativamente simples, tal como ilustrado abaixo.


```{r}
#SSE - Soma dos quadrados dos erros -- intercept-only
dados_brasil <- dados_brasil %>% 
  mutate(erros_intercepto_apenas = residuals(mod_intercepto_apenas)^2)
...
No Modelo Aumentado, o SSE é de 163401. Vale lembrar que a previsão dos valores de $\hat{y_i}$ deSSR modelo conta com os resultados da variável `bsq_soma` 
...
$PRE = \frac{299560.7-163401}{299560.7}  = 0.4545318$
...
f_critical <- ggplot(data.frame(x=c(0,5)), aes(x=x)) +
...
f_calculated <- ggplot(data.frame(x=c(0,5)), aes(x=x)) +
     stat_function(fun=df, args=list(df1=1, df2=217), colour="blue", size=0.5) +
     geom_vline(xintercept = qf(1-0.05, 1, 217)) +
...
grid.arrange(f_critical, f_calculated)
```


Agora, após esses cálculos, é possível concluir que a adição de um novo preditor no modelo reduz significativamente o erro presente em um modelo que conte apenas com a média (F(1, 217) = 180.8, p < 0.01). Posto isso, também é possível apresentar a função nativa `anova` que o R executa ao realizarmos a regressão. Repare que os valores encontrados são exatamente os mesmos (299561 para o SSE do Modelo Compacto e 163401 para o SSE do Modelo Aumentado). O valor de F também e p também são os mesmos (180.82 e p < 0,01).
...
```{r}
include_graphics("raw_img/cap_reg_homocedasticidade.png")
```
