...
A ANOVA é um teste estatístico desenhado para verificar diferença entre diversos grupos e, funcionalmente, é possível entender ANOVA como um super Teste T. Conceitualmente, no entanto, ela é um caso especial de um modelo de regressão em que a variável independente é categórica. Em Psicologia, provavelmente a ANOVA é a técnica inferencial mais utilizada [@Chartier2008; @Howell2011]. Por um aspecto, isso é extremamente vantajoso, dado que essa técnica estreita a relação entre Psicologia e Estatística. Em outro, isso parece ter contribuído para criação e manutenção de diferentes conceitos equivocados sobre a ANOVA. 
...
Neste capítulo, vamos utilizar a pesquisa intitulada "A relação entre o nível de Empreendedorismo (TEG) e os aspectos sociodemográficos dos Taxistas cooperados da cidade de Santo André/São Paulo, Brasil", publicada em 2016 na Revista Eletrônica de Gestão e Serviços, que sou co-autor.
...
Conforme foi comentado, os itens em um instrumento sociodemográfico devem levar em consideração o contexto das pessoas avaliadas e, por isso, as categorias de escolaridade foram definidas como as apresentadas. Primário significa escolaridade até o 5 ano, ginásio significa escolaridade até o 9 ano e colegial é equivalente ao ensino médio.


vamos iniciar as análises a partir de tabelas e gráficos. 
...
Uma vez que a escolaridade (VI) é tratada como discreta e a TEG (VD) é contínua, um gráfico de barras é adequado. A inclusão das barras de erro permite uma compreensão inferencial inicial.
...
O gráfico permite identificar algumas caracteríticas gerais dos resultados. Primeiro, quão maior a escolaridade, maior o o valor obtido na escala. Segundo, algumas barras de erros estão superpostas e outras não, o que nos leva à conclusão de que resultados significativos estarão presentes na próxima etapa, que é a modelagem formal dessa hipótese.
...
A função `lm` será utilizada e o objeto `mod_escolaridade` será armazenado.
...
Antes de checar os resultados, é necessário verificar os pressupostos da ANOVA, que pedem que os resíduos sejam normalmente distribuídos e homocedásticos. Essa apresentação será feita somente agora para que os outros tópicos sejam mais claros e assertivos. 

Gráficos e testes formais são muito úteis para verificar ambos os casos e vamos começar pela normalidade.
...
E o teste formal pode ser feito via `shapiro wilk`. Lembre-se que a $H_0$ desse teste estipula normalidade e, idealmente, não deve ser rejeitada.

```{r}
shapiro.test(residuals(mod_escolaridade))
```

É possível concluir que o pressuposto da normalidade foi violado. situações como essa são muito frequentes. Na literatura, são presentes recomendações de ajuste dos dados por alguma função `g(.)` ou tornar mais severo o valor de p na interpretação dos resultados. Por exemplo, em vez de 0.05, usar 0.01.

A homocedasticidade pode ser verificada por um gráfico dos resíduos dado valores previstos. 
...
Além do teste de `Levene`, que estipula $H_0$ como homocedasticidade e, idealmente, não deve ser rejeitada.
...
A homocedasticidade foi garantida. Dessa maneira, mesmo com a violação da normalidade, o modelo será utilizado e seu sumário geral será apresentado pela função Anova do pacote `car`. Isso é importante para garantir resultados programas tipicamente utilizados, como o SPSS e o STATA. Repare que como o pacote não foi formalmente carregado, ele será acionado indiretamente incluindo `::` após seu nome.

```{r}
car::Anova(mod_escolaridade, type = 3)
```

A leitura desse resultado é similar a de um modelo de regressão. Dessa maneira, esse modelo aumentado (MA), ao ser comparado com o modelo compaco/nulo (MC), foi significativo (F(3,143) = 8.231, p < 0.01). Isso significa que a adição do preditor foi significativamente capaz de reduzir o erro do modelo nulo. De fato, quando isso ocorre, a busca por possíveis diferenças entre **todas as comparações possíveis** é feita.

É importante mencionar que isso não significa que se uma ANOVA é significativa, haverá alguma diferença entre as médias de grupos. Tecnicamente, a diferença pode ocorrer em qualquer comparação possível, como grupo 1 contra grupos 2 + grupo 3. Dessa maneira, o resultado geral da ANOVA e testes post-hoc respondem questões diferentes e é possível realizar qualquer comparação múltipla sem sequer realizar a ANOVA, desde que os resultados sejam devidamente corrigidos.

Uma vez que a`escolaridade` foi significativa (F(3, 143) = 8.231, p < 0.01) mesmo em uma significância mais estrita (0.01), vamos checar as comparações principais. Estas são aquelas relacionadas a cada um dos níveis presentes nessa variável. Uma vez que comparações serão realizadas, é necessário ajustar o valor de P. Repare que a quantidade de comparações pode ser calculada da seguinte forma:
...
Para comparação pareada, o resultado será armazenado em um objeto específico nomeado como `post_hoc_escolaridade`. Isso será útil para apresentar sumários e gráficos. O sumário ajustará o valor de p pela técnica Bonferroni, que multiplica o valor bruto encontrado pela quantidade de comparações.
...
base_interacao <- base_interacao %>% 
  mutate_at(vars(estado_civil), funs(
    factor(case_when(
      . == 1 ~ "Solteiro",
