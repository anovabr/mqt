...
```
É importante ter atenção que este gráfico não é necessariamente o melhor para uma ANOVA de 2 vias, uma vez que pode sugerir um relacionamento entre as variáveis independentes. Esse aspecto será melhor debatido na seção de ANOVA fatorial.  
...
A base a ser carregada é [CSV file - MEMORE Cognitive measurement.csv](https://osf.io/4hdc2/). Após carregar tal base, a seção `Descriptives` apresentará o gráfico inicial dos resultados.
...
Em seguida, ao clicar na opção Plots, será possível selecionar o `Boxplot` e `Boxplot element`. O gráfico aparecerá abaixo da tabela e irá apresentar diferentes informações estatísticas da distribuição dos resultados da avaliação psicológica em função dos níveis de <u>escolaridade_grupo</u>. Uma visualização preliminar indica que pessoas com escolaridade mais elevada (níveis 2 e 3) apresentam resultados maiores do que pessoas com o primeiro nível de escolaridade.
...
Na ANOVA de duas vias, o post hoc será realizado para cada um dos níveis ou fatores do modelo. Neste caso, <u>escolaridade_grupo</u> e <u>faixa_etaria</u>. 


## Execução no R 

O pacote `emmeans` será utilizado para, inicialmente, verificar cada uma das comparações entre os níveis de <u>escolaridade_grupo</u>. É bom notar que os resultados são ajustados pelas outras variáveis que integram o modelo. O vetor `post_hoc_twoway_escolaridade` reunirá os resultados.
...
O gráfico a seguir foi realizado também na ANOVA de 1 via e apresenta um resultado mais fácil de interpretar em relação às comparações. O eixo horizontal tem destaque ao valor `0` e o eixo vertical apresenta todas as comparações. 
...
O gráfico, novamente, é um recurso útil à visualização preliminar dos resultados.  
...
Por sua vez, a tabela apresenta os resultados de maneira detalhada.
...
Em seguida, inserir as variáveis para o lado direito. O JASP apresentará todas as comparações feitas em <u>escolaridade_grupo</u> e <u>faixa_etaria</u>, ajustando os resultados por todas as variáveis no modelo e corrigindo o valor de P. Por padrão, a correção de P é feita pelo método de `Tukey`, que pode ser alterada na seção `Correction`. 

![](./img/cap_anova_two_way_posthoc2.png)
Mesmo sem implementar a correção de Bonferroni, os achados são virtualmente idênticos aos obtidos anteriormente pelo R. Possíveis diferenças de sinal (+ ou -) ocorrem pela codificação das variáveis e não impactam em nada a interpretação dos resultados. Para inserir também um gráfico característico dos resultados, basta clicar em `Descriptives Plots`.   

Muitos gráficos são possíveis, mas a maioria irá destacar a possível relação que <u>escolaridade_grupo</u> e <u>faixa_etaria</u> possuem, o que será realizado na seção de ANOVA fatorial. 
...
A ANOVA Fatorial é uma ANOVA de 2 (ou mais) vias, em que se estipulam interações entre os fatores. O conceito de interação significa que há dois ou mais fatores e o efeito de um fator no desfecho depende do nível dos outros fatores. Na prática, na maior parte do tempo que um pesquisador pensa em uma ANOVA de 2 vias, o interesse justamente estar em saber se os fatores apresentam (ou não) uma interação. Talvez seja justamente por isso que a maioria dos programas comerciais realizam uma ANOVA Fatorial por padrão quando se solicita uma ANOVA de 2 vias.  
