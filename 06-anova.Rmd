...
A ANOVA é um teste estatístico desenvolvimento para verificar diferença entre diversos grupos. Pragmaticamente, é possível entender ANOVA como um super Teste T, onde o que está em jogo não é somente o quanto as médias amostras estão distantes, mas o quão distantes estão relativamente à variabilidade de observações individuais. Conceitualmente, ela é um caso especial de um modelo de regressão em que a variável independente é discreta/categórica. 
...
Em seguida, a apresentação tabularr das médias e desvios-padrão pode ser realizada.
...
A homocedasticidade pode ser verificada por um gráfico dos resíduos dado valores previstos.  O ideal é não encontrar padrões
...
A leitura desse resultado é similar a de um modelo de regressão. Em linhas gerais, quase sempre apenas se diz que o modelo foi significativo (F(3,143) = 8.231, p < 0.01). No entanto, o detalhamento desta condição impõe comentar que a adição da variável "escolaridade" no  modelo testado (chamado de modelo aumentado - MA) foi significativamente capaz de reduzir o erro do modelo nulo, que e o modelo que apenas utiliza a média como preditor (também chamado de modelo nulo ou compacto neste caso - MC). 

De fato, quando isso ocorre, a busca por possíveis diferenças entre <u>todas as comparações possíveis</u> é feita.
