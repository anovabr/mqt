...
$$H_0: \mu_{escolaridade_i} = \mu_{escolaridade_j} \\ H_a: c.c \\ \alpha = 0.05$$

Aqui, os subscritos $i$ e $j$ foram utilizados de maneira liberal para apresentar a diferença entre todas as combinações lineares possíveis.
...
Mesmo sem implementar a correção de Bonferroni, os achados são virtualmente idênticos aos obtidos anteriormente pelo R. Possíveis diferenças de sinal (+ ou -) ocorrem pela codificação das variáveis e não impactam em nada a interpretação dos achados. Para inserir também um gráfico característico dos resultados, basta clicar em `Descriptives Plots`.   

Muitos gráficos são possíveis, mas a maioria irá destacar a possível relação que <u>escolaridade</u> e <u>faixa_etaria</u> possuem, o que será realizado na seção de ANOVA fatorial. 
...
3. A interpretação dos resultados de um fator é ajustada pelo outro  
4. Os post hocs devem ser feitos individualmente para cada fator  
