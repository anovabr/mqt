...
load(file="~/anovabr/mqt/bases/Livro - R - ASQ SE all age intervals.RData")
...
```{r}
effsize::cohen.d(score ~ sex, data = asq_18months)
```
...
Dessa maneira, quando se comenta sobre o princípio da normalidade no teste T, o que se está falando é sobre a normalidade dos resíduos deste modelo. Isso pode ser feito tanto em formato gráfico:

```{r}
plot(lm(score ~ sex, data = asq_18months),2)
```
  
Como por testes formais, tal como o Anderson-Darling:

```{r}
nortest::ad.test(lm(score ~ sex, data = asq_18months)$residuals)
```
