...
A chamada versão paramétrica do teste T é o teste de Wilcoxon-Mann-Whitney. 

```{r}
(mann_whiyney_18m <- wilcox.test(score ~ sex, data = asq_18months))
```

O tamanho do efeito também pode ser calculado ao implementar $Z/sqrt(n)$. O pacote `coin` traz essa métrica

```{r}
#coin::wilcox_test(score ~ sex, data = asq_18months)[[teststatistic]]
```
