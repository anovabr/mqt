...
Dessa maneira, o estudo das variáveis (aleatórias) é central em estatística, seja tanto para análises descritiva,s como inferenciais.   
...
Os termos VI e VD explicitam que ambas as variáveis são contectadas e que a VI gera ou causa a VD. Neste sentido, representações matemáticas apresentam a VI por X e a VD por Y, tal como no estudo de funções.  
...
É possível afirmar que em todos os relacionamentos entre duas variáveis, sempre havera um conjunto de outras variáveis impactando esta relação. Conseguir mapear essas variáveis depende tanto da maturidade científica de uma área, como do objetivo que a pesquisa se propõe. É possível organizar as pesquisas a partir de três principais objetivos, que são as (1) pesquisas exloratórias, (2) pesquisas descritivas e (3) pesquisas explicativas. A tabela a seguir sintetiza as principais características de cada um desses tipos.    
...
Portanto, durante o mapeamento do relacionamentos entre VI, VD e todas as possíveis variáveis estranhas que impactam neste primeiro relacionamento, o objetivo definido para a pesquisa serve como primeiro marcadador neste sentido. No entanto, é durante o planejamento da pesquisa em sua dimensão mais ampla, mas especialmente focado nas condições de coleta e análise de dados, é que o pesquisador terá a oportunidade de eleger as maneiras pelas quais ele irá executar seu estudo e mapear as principais características e limitações existentes. O conceito de delineamento de pesquisa é justamente utilizado para expressar esta ideia, sendo formalmente entendido como um manual ou roteiro que "considera o ambiente em que são coletados os dados e as formas de controle das variáveis envolvidas" [@gil2002]. 


Existem diferentes classificações dos delineamentos de pesquisa e cada área tem particularidades. Definição claramente o tipo de delineamento é algo fundamental, já que as técnicas e análises estatísticas são conectadas a eles. Em Psicologia, costuma-se organizar os delinemaneot em observacionais, correlacionais e experimentais. As vezes, o termo delineamento descritivo é utilizado em vez de observacional [@stangor2010]. Por sua vez, em epidemiologia, saúde pública e bioestatística, os delineamentos costumam ser divididos em experimentais e observacionais [@friis2013]. Apesar das diferenças de nomenclaturas, a lógica por detrás dos delineamentos é bastante atrelada à possibilidade e viabilidade de uma manipulação ativa da VI. 
...
Em delineamentos observacionais (ou observacionais e correlacionais), o pesquisador não intervem nos fenômenos, o que significa que, neste delineamento, não há uma manipulação da VI. Neste tipo de estudo, com frequência um único grupo é avaliado e os resultados auxiliam em um entendimento inicial do fenômeno ou objeto de interesse. Diferentes técnicas estatísticas podem ser utilizadas, mas em momento algum é possível falar sobre relação de causa e efeito.   


Uma vez que a formação de grupos seguindo um rigoroso controle da VI não foi feito, as variáveis estranhas podem assumir diferentes papéis no relacionamento da VI e VD.  Alguns autores defendem que quando esta outra variável é uma consequência da VI e também impacta a VD, o termo variável mediadora deveria ser empregado. Por exemplo, ao se investigar a relação entre idade (VI) e acidentes de transito (VD), pode-se sugerir que pessoas mais velhas tendem a apresentar mais problemas visuais que, por sua vez, também pode gerar acidentes de transito. Neste sentido, a variável problemas visuais é uma variável mediadora.  
...
Quando esta variável interfere no relacionamento VI e VD, podendo alterar sua natureza e intensidade, mas não é gerada pela VI, o termo variável moderadora seria adequado. Por exemplo, a relação entre horas trabalhadas (X) e quantidade de trabalho feita (Y) é moderada pelo equipamento (e sua qualidade). É possível que alguém gaste muitas horas de trabalho, mas não consiga produzir absolutamente nada, uma vez que os equipamentos utilizados funcionam de maneira irregular ou não funcionam. No exemplo prévio em relação à idade (X) e acidentes de trânsito, o sexo é sabidamente uma variável moderadora. A literatura, inclusive, sugere que pessoas mais novas, quando homens, tem uma alta probabilidade de se envolverem em acidentes de trânsito, especialmente os mais intensos ou graves.  

  
![](./img/cap_moderacao.png)

É também possível que esta variável interfira tanto na VI como na VD. O termo terceira variável ou variável de confundimento deveria ser empregado. Por exemplo, existe uma alta correlação entre venda de sorvete (X) e morte por mordida de tubarão (Y). Esse tipo de correlação é chamada de espúria, uma vez que é a temperatura que está gerando ambos os fenômenos. Talvez quando mais novo(a), alguém lhe comentou que as cegonhas traziam os bebês. De fato, isso é um bom exemplo de variável de confundimento. A relação entre número de ninhos de cegonha (X) e número de bebes nascidos (Y) é confundida pela temperatura do local 9 meses atrás. De fato, cegonhas e seres humanos tem seus filhos/filhotes na mesma época. Curiosamente, como as cegonhas por vezes construiam seus ninhos em telhados de casas, especialmente as com chaminés por conta do calor, as pessoas acreditavam que se uma cegonha fizesse o ninho em seu telhado, isso indicava que o casal teria um filho e, em alguns países, também se achava que deixar doces na janela seria uma maneira de informar as cegonhas que a casa estava pronta para ter um bebê..
...
Finalmente, quando a variável é psicológica, como motivação, o termo variável interveniente deveria ser empregado. 




Investigar habitos de pesquisa [@Wenke2017], alimentação durante a pandemia [@Amatori2020] ou aspectos epidemiológicos de universitários [@AfonsoJunior2020] exemplificam bem este delineamento.
...
Portanto, a integração do tipo de pesquisa e tipo de delineamento sevem como passos fundamentais para execução da pesquisa, bem como criam um indicador em que é possível avaliar o quanto os resultados são acurados. 
