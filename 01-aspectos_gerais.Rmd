...
Ainda neste sentido, mapas cartográficos ilustram bem a importância dos modelos. Apesar de todos os mapas disponíveis distorcerem um pouco a área e localização dos continentes, a utilização deles é fundamental tanto na ciência como no dia-a-dia das pessoas.  

![](./img/cap_mapas.png) 

 
Por sua vez, os instrumentos de medida são recursos tecnológicos que permitem uma avaliação mais detalhada do fenômeno ou do objeto de investigação. A utilização de instrumentos também gera um conjunto de dados que podem ser analisados, comparados e compartilhados. Todas as áreas científicas desenvolvem e refinam instrumentos de medida e há autores que sugerem que a existência e a qualidade de tais recursos indicam o quão madura e desenvolvida uma área se encontra. Com muita frequência, o registro que temos da realidade é baseado em nossos órgãos sensoriais que, por sua vez, são influenciados pela cultura em que estamos inseridos e pela aprendizagem prévia pela qual passamos.   
...
O segundo conceito se dá na dicotomia constante vs. variáveis. De maneira geral, um fenômeno constante não apresenta dispersão em seus valores. Em oposição, fenômenos variáveis podem assumir quaisquer valores em suas características. Enquanto a velocidade da luz e do som é, respectivamente, 324 m/s e 299.792 m/s; a renda, altura ou inteligência das pessoas podem variar bastante. De fato, essa dicotomia também poderia ser vista pelo conceito de modelos determinísticos e probabilísticos ou, superficialmente, em algumas diferenças entre ciências naturais e ciências humanas. Modelos determinísticos são aqueles que as condições iniciais determinam os resultados, enquanto modelos probabilísticos são os que associam probabilidades aos resultados. Por vezes, em ciências naturais recorre-se menos à noção de probabilidade, enquanto em ciências humanas este é um conceito central.  
...
Os termos VI e VD explicitam que ambas as variáveis são contectadas e que a VI gera ou causa a VD. Neste sentido, representações matemáticas costumam apresentar a VI por X e a VD por Y, tal como no estudo de funções.  

![](./img/cap_vi_vd1.png)  

Em alguns fenômenos naturais, é relativamente fácil identificar a VI e a VD. Por exemplo, quanto mais profundo uma pessoa mergulha no mar, mais gelada a água fica. Neste caso, a profundidade seria a VI, enquanto a temperatura seria a VD. Da mesma forma, quanto mais alto um avião viaja, menor é a pressão atmosférica. Nesse outro exemplo, a altitude é a VI e a pressão atmosférica é a VD. 

![](./img/cap_vivd_fisica.png) 


No entanto, esses exemplos descritos são oportunos e têm apenas finalidade pedagógica. Na realidade, a definição de VI e VD nem sempre é fácil e, especialmente em ciências humanas e da saúde, isso se torna ainda mais sutil. É possível definir tais variáveis a partir de condições temporais, lógicas ou teóricas [@BabbieEarlR1990], bem como é possível desenvolver pesquisas com características específicas para que essa relação possa ser definida com acurácia, e consequentemente, corretamente medida. 
...
É importante notar que a ideia de uma única variável impactando ou causando diretamente uma outra variável é quase metafórica. Para que isso pudesse ocorrer tanto em fenômenos físicos como em quaiquer outros, seria necessário que todas as possíveis fontes de influência existentes neste relacionamento fossem controladas ou suprimidas [@Dumsday2012]. Essa noção de controle total é visto pelo termo *ceteris paribus*, que é uma expressão em latim que significa "tudo o mais constante". Muitas vezes, em pesquisa e estatística, este termo é utilizado quando alguns ajustes são implementados em modelos estatísticos para tentar expressar a ideia de uma causalidade parcial, em que a VI impactaria a VD de uma determinada forma, caso tudo o mais fosse controlado [@Fennell2005]. Evidentemente, há condições encontradas com certa regularidade em fenômenos e objetos sociais e o termo "fato estilizado" tende a ser empregado nestas situações [@Hirschman2016].  


Ao se pensar em condições em que a VI e VD estejam definidas, é fácil notar que existe um conjunto grande de outras variáveis que podem impactar neste relacionamento. Essas variáveis recebem muitos nomes e o termo <u>variáveis estranhas</u> pode servir para designá-las. A imagem a seguir ilustra algumas das variáveis que poderiam impactar na relação entre horas de estudo e notas na prova.
 

![](./img/cap_variavel_estranha.png)

A natureza desta variável estranha e o local em que ela impacta no relacionamento entre a VI e a VD entram em cena quando conceitos específicos são estudados. No capítulo sobre correlação e regressão essa discussão será revisitada.   

Alguns autores defendem que quando esta outra variável é uma consequência da VI e também impacta a VD, o termo variável mediadora deveria ser empregado. Quando esta variável interfere no relacionamento VI e VD, podendo alterar sua natureza e intensidade, mas não é gerada pela VI, o termo variável moderada seria adequado. Quando a variável influencia tanto a VI como a VD, o termo terceira variável ou confundidora deveria ser empregado. Finalmente, quando a variável é psicológica, como motivação, o termo variável interveniente deveria ser empregado. 

Abaixo há uma ilustração que sintetiza esses conceitos, com exemplos pontuais. 
...
A literatura é bastante rica de exemplos para tais condições, mas também de discussões e debates acadêmicos sobre esses conceitos. A tabela a seguir reúne esforços e exemplifica onde tais conceitos são implementados.  
  ...
  | Mediação             | Relação entre conscienciozidade (X) e saúde (Y), comportamentos saudáveis poderia ser mediador   
...
É possível notar que estes são exemplos gerais e úteis para introduzir o tema. Eles descrevem condições pelas quais as variáveis se relacionam ou se conectam, bem como os diversos cenários de complexidade desse relacionamento. No entanto, eles contam apenas uma parte da história, já que não abrangem as principais estratégias que os pesquisadores possuem para lidar com tais condições em suas pesquisas.  


Na realidade, quando pesquisadores visam estudar o relacionamento entre variáveis, todas essas condições costumam ser levadas em consideração antes mesmo da coleta de dados pelo que é chamado de delineamento de pesquisa. Este termo é definido como a "parte do planejamento da pesquisa que envolve tanto a diagramação quanto a coleta de dados, com função de possibilitar que novos pesquisadores executem a mesma pesquisa e possam chegar às mesmas conclusões" [@gil2002]. Assim, o delineamento de pesquisa serve tal como um manual ou roteiro amplo da pesquisa, com especial foco nos procedimentos técnicos de coleta e análise de dados e que mapeia as principais características e limitações existentes durante o estudo. 


Existem diferentes classificações dos delineamentos de pesquisa e cada área tem particularidades. A definição do tipo de delineamento é algo importante, uma vez que técnicas e análises estatísticas são conectadas a eles. Em Psicologia, frequentemente se encontra delineamentos observacionais, correlacionais e experimentais e, as vezes, o termo delineamento descritivo é utilizado em vez de observacional [@stangor2010]. Por sua vez, em epidemiologia, saúde pública e bioestatística, os delineamentos costumam ser divididos experimentais e observacionais [@friis2013]. Apesar das diferenças de nomenclaturas, a lógica por detrás dos delineamentos é bastante atrelada à possibilidade e viabilidade de uma manipulação ativa da VI. 
...
Em Psicologia, a tabela abaixo é bastante utilizada:
  ...
  | :-----------    | :-----------                                | :-----------              |  :-----------      
  | Observacional   | Apenas 1 grupo é necessário. Descrever um determinado fenômeno ou objeto | Oferece uma foto de um objeto ou fenômeno que tende a ser precisa e permite que novos estudos sejam conduzidos  |  Não acessa o relacionamento entre duas ou mais variáveis    
  | Correlacional    | Apenas 1 grupo é necessário. Acessar a natureza e a força do relacionamento entre duas variáveis | É possível testar hipóteses sobre este relacionamento e fazer previsões de resultados futuros | Não permite indicação de causalidade  
  | Experimental     | São necessários 2 grupos em que os participantes sejam selecionados de maneira aleatória. Existe uma manipulação intencional da VI e nem os participantes nem o pesquisador sabe quem está em cada grupo (duplo cego)  | A aleatorização evita que as variáveis estranhas gerem viéses e a comparação entre os grupos permite conclusões sobre causalidade |  Impossível, impraticável ou até mesmo antiético em algumas situações.  O custo envolvido costuma ser alto.  


Em Epidemiologia, saúde pública e bioestatística, costuma-se unir o delineamnento observacional e correlacional, em uma configuração um pouco distinta:

  | Delineamento    | Característica                              | Vantagens               |  Limitações    
  | :-----------    | :-----------                                | :-----------              |  :-----------      
  | Observacional   | Apenas 1 grupo é necessário. Descrever um determinado fenômeno ou objeto, bem como investigar a relação entre eles | Oferece uma foto de um objeto ou fenômeno que tende a ser precisa, permite que novos estudos sejam conduzidos e é possível testar hipóteses sobre este relacionamento e prever valores futuros  |  Não acessa o relacionamento entre duas ou mais variáveis    
  | Experimental     | São necessários 2 grupos em que os participantes sejam selecionados de maneira aleatória. Existe uma manipulação intencional da VI e nem os participantes nem o pesquisador sabe quem está em cada grupo (duplo cego)  | A aleatorização evita que as variáveis estranhas gerem viéses e a comparação entre os grupos permite conclusões sobre causalidade |  Impossível, impraticável ou até mesmo antiético em algumas situações.  O custo envolvido costuma ser alto.  


De maneira geral, delineamentos observacionais (ou observacionais e correlacionais) são aqueles em que o pesquisador não intervem nos fenômenos, ou seja, não há uma manipulação da VI. Nestes estudos, com frequência um único grupo é avaliado e os resultados auxiliam em um entendimento inicial do fenômeno ou objeto de interesse. Diferentes técnicas estatísticas podem ser utilizadas, mas em momento algum é possível falar sobre relação de causa e efeito.  Investigar habitos de pesquisa [@Wenke2017], alimentação durante a pandemia [@Amatori2020] ou aspectos epidemiológicos de universitários [@AfonsoJunior2020] exemplificam bem este delineamento.

Delineamentos experimentais ocorrem quando há uma manipulação intencional do pesquisador sobre a VI, dois grupos foram aleatoriamente formados e nem o participante, nem o pesquisador sabem em que grupo cada um está (duplo cego). A aleatorização neste tipo de pesquisa impossibilita ou dificulta que as variáveis estranhas ocasionem viés nos resultados e a comparação entre os grupos podem indicar resultados causais. Estudar o efeito da exposição à violência no comportamento agressivo durante a interação social de crianças [@Bandura1961], o tipo de cor de pele na possibilidade ou não de ganhar uma carona no ônibus [@Mujcic2020] ou [Alguma pequisa do Landeira] ilustra bem uma delineamento experimental.
...
Neste sentido, uma vez que um dos principais interesses das ciências é o estudo de variáveis, é bastante infrequente que cientistas digam que estão certos de uma determinada condição ou assunto. Na maioria das vezes, cientistas substituem a palavra certeza por probabilidade.
